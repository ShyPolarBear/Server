...
public class QuizTemplate {
    ...
    public static OXQuiz createDummyOXQuizF() {
        return OXQuiz.builder()
                .question("폐형광등, 건전지는 일반 쓰레기다")
                .answer(OXChoice.X)
                .explanation("폐건전지가 든 일반 쓰레기를 태우면 폭발위험이 있으므로 주민센터 / 행정 복지 센터 / 폐건전지 수거함에 버려주세요!")
                .build();

    }

    public static OXQuiz createDummyOXQuizG() {
        return OXQuiz.builder()
                .question("야간 인공조명은 동식물 성장에 해롭다")
                .answer(OXChoice.O)
                .explanation("야간 인공조명은 동식물의 생태계를 파괴시키는 빛공해에 포함되어 정상적인 성장에 악영향을 끼칩니다")
                .build();
    }
    ...
    // 객관식 선택지 4개
    public static final MultipleChoice dummyMultipleChoiceAA = MultipleChoice.builder().multipleChoiceQuiz(dummyMultipleChoiceQuizA).isAnswer(false).content("견과류 껍데기").build();
    public static final MultipleChoice dummyMultipleChoiceAB = MultipleChoice.builder().multipleChoiceQuiz(dummyMultipleChoiceQuizA).isAnswer(false).content("일회용 티백").build();
    public static final MultipleChoice dummyMultipleChoiceAC = MultipleChoice.builder().multipleChoiceQuiz(dummyMultipleChoiceQuizA).isAnswer(true).content("바나나 껍질").build();
    public static final MultipleChoice dummyMultipleChoiceAD = MultipleChoice.builder().multipleChoiceQuiz(dummyMultipleChoiceQuizA).isAnswer(false).content("계란 껍데기").build();

    public static List<MultipleChoice> createDummyMultipleChoiceList() {
        return Arrays.asList(dummyMultipleChoiceAA, dummyMultipleChoiceAB, dummyMultipleChoiceAC, dummyMultipleChoiceAD);
    }
